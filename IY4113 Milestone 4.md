...
### 02/10/2026 - Diary Entry 7 – Remove journey and Journey class

For today, as I was quite packed with a busy schedule, I only focused on remove journey function and adding the "Journey" class. This part was one of the most important step, because of the usage of journey class.

For my remove journey method, I added safety confirmation, journey identification and recalculation. It asks the user to enter a journey ID, then it loops through the all recorder journeys searching for the matching ID. If there is no match, it sends out an error message. Then it asks the user to confirm the removal before removing it from the journeys list. Then it recalculates the running total using recalculateTotals method. thos method clears out all the current totals and resets the journeys list. Then it recalculates the all remaining journeys. In the end, it assigns new unique IDs to the remaining journeys.

---

### 02/14/2026 - Diary Entry 8 – Initialize CityRideDataset

For today, I implemented pre-built java class into my code. It was quite simpler than I expected. However, as my previous code was not built for the CityRideDataset, I had to refactor my addJourney and setupSession.

My "addJourney" ensures that the starting zone is in between 1-5 and the time band is either peak or off-peak. It uses abs(toZone - fromZone) + 1 calculations to calculate the crossed zones. This method also catches the discount rates from the dataset and implements it to the running total. It applies the discount before checking the daily cap. Most important part is it creates a new journey object with an unique ID and adds it to the journeys list.

My "setupSession" captures the passenger name and sets up the passenger type. Getting the passenger type is crucial for the discoutn rate. In the end, it outputs a confirmation feedback for the user.

---

### 02/15/2026 - Diary Entry 9 – Filtering journeys
